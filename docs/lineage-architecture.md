...
lineage and store explanations separately in the analysis Git repository.
Repotraverse does not produce stability labels or scores.
...
header records the repository and traversal parameters, each `integration_unit` records
...
inputs for inference, not stability conclusions.
...
`repotraverse/v1/coordination/tasks/<producer-id>`, so every VM discovers the same work without
connecting to another VM. Before extraction, a VM must atomically create the
corresponding `repotraverse/v1/coordination/claims/` ref. Claims expire and are renewed or taken
