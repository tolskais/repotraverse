...
$sbom = [ordered]@{
    ...
    documentNamespace = "https://internal.invalid/repotraverse/$preset/$([guid]::NewGuid())"
    creationInfo = @{ created = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ"); creators = @("Tool: package-windows.ps1") }
    packages = @(
        @{ name = "repotraverse"; SPDXID = "SPDXRef-Package-Repotraverse"; versionInfo = "1.0.0"; downloadLocation = "NOASSERTION"; filesAnalyzed = $false; licenseConcluded = "NOASSERTION"; comment = $extractorVersion },
